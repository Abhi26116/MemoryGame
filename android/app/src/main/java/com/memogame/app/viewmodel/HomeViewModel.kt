...
/**
 * Derived home/levels data over the ProgressStore. Because the store's fields
 * are Compose state, reading these properties inside a composable recomposes
 * automatically on progress changes.
 */
class HomeViewModel(private val progressStore: ProgressStore) {

    val levels: List<LevelModel> get() = LevelCatalog.allLevels

    /**
     * Only the levels the player has reached, plus the next locked one as a
     * teaser — so the total number of levels stays a mystery.
     */
    val visibleLevels: List<LevelModel>
        get() {
            if (ALL_LEVELS_UNLOCKED) return levels
            val result = mutableListOf<LevelModel>()
            for (level in levels) {
                result.add(level)
    ...
    val totalStars: Int get() = progressStore.totalStars
    val completedLevels: Int get() = progressStore.completedLevels
    val totalLevels: Int get() = LevelCatalog.LEVEL_COUNT

    fun stars(levelId: String): Int = progressStore.progress(levelId)?.stars ?: 0

    fun isCompleted(levelId: String): Boolean =
        (progressStore.progress(levelId)?.completedCount ?: 0) > 0

    fun isUnlocked(level: LevelModel): Boolean {
        if (ALL_LEVELS_UNLOCKED) return true
        if (level.levelNumber <= 1) return true
        val previous = LevelCatalog.level(level.levelNumber - 1) ?: return false
        return isCompleted(previous.id)
    }

    fun unlockHint(level: LevelModel): String? {
        if (isUnlocked(level) || level.levelNumber <= 1) return null
        val previous = LevelCatalog.level(level.levelNumber - 1) ?: return null
        return "Complete Level ${previous.levelNumber} to unlock"
    }

    /** Next unlocked level the player hasn't finished yet (for Continue). */
    val suggestedLevel: LevelModel?
        get() = levels.firstOrNull { isUnlocked(it) && !isCompleted(it.id) }

    /**
     * Offline "Daily Highlight" — a memory tip that rotates once per calendar
     * day. 100 tips means a full ~100-day cycle before any repeat. Kept in
     * sync with iOS's copy of the same list (`HomeViewModel.swift`) so a
     * given calendar day shows the same tip on both platforms.
     */
    val dailyTip: String
        get() {
            val day = Calendar.getInstance().get(Calendar.DAY_OF_YEAR)
            return DAILY_TIPS[day % DAILY_TIPS.size]
        }

    companion object {
        /**
         * Testing override: set true to play every level without earning stars first.
         * Off in production — only Level 1 is open; each level unlocks by completing
         * the previous one.
         */
        const val ALL_LEVELS_UNLOCKED = false
